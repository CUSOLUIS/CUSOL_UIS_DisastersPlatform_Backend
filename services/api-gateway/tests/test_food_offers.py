"""CHG-163 — Gateway de «Ofrecer comida».

El alta y el listado son públicos: la cuenta es opcional y viaja en
headers internos; el alta comparte los limitadores de contribución de
los reportes ciudadanos y la lectura tiene el suyo. Nunca se reenvían
cookies al servicio interno.
"""
...
PATH = "/api/v1/food-offers"
IDEMPOTENCY = {"Idempotency-Key": "clave-idempotente-0163"}
OFFER_ID = "88888888-8888-4888-8888-888888888801"

RECEIPT = {
    "id": OFFER_ID,
    "publicCode": "FO-2026-BBBB2222",
    "status": "active",
...
async def test_create_forwards_json_and_hides_cookies():
    ...
    assert seen["path"] == "/internal/v1/food-offers"
...
async def test_list_has_its_own_read_limit():
    ...
    app = create_app(
        gateway_settings(food_offer_read_rate_limit_per_minute=1),
        upstream,
        identity,
    )
